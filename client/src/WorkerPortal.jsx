...
export default function WorkerPortal({ tickets = [], onNotice, onTicketUpdated }) {
  ...
  const loadAr = async () => {
    if (!task) return
    try { setAr(await api.arProcedure(task.fault || task.issue)) } catch { setAr({ mode: 'fallback', steps: defaultSteps }) }
  }
