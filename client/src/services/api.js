...
export const api = {
  ...
  setWorkerStatus: (id, status) => request(`/workers/${id}/status`, { method: 'PATCH', body: JSON.stringify({ status }) }),
  ...
  arProcedure: (fault) => request(`/ar/procedures/fault/${encodeURIComponent(fault)}`),
  arProcedureForTicket: (ticketId) => request(`/ar/procedures/${ticketId}`),
  ...
  setTaskStatus: (id, status, notes = '') => request(`/tasks/${id}/status`, { method: 'PATCH', body: JSON.stringify({ status, notes }) }),
